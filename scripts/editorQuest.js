...
const questionCreator = () => {
  ...
  QuestionPlainF.forEach(plan => {
    console.log([plan]);
    var a =
      plan.lastElementChild.firstElementChild.className === "answer correct"
        ? true
        : false;
    var b =
      plan.lastElementChild.firstElementChild.nextElementSibling.className ===
      "answer correct"
        ? true
        : false;
    var c =
    ...
    var d =
      plan.lastElementChild.firstElementChild.nextElementSibling
        .nextElementSibling.nextElementSibling.className === "answer correct"
        ? true
        : false;
    // var img = plan.lastElementChild.firstElementChild
    questionRack = [
      ...questionRack,
      new Question_and_answer(
        parseInt(plan.firstElementChild.firstElementChild.innerText, 10),
        ...
        c,
        d,
        "" // plan.lastElementChild.previousElementSibling.firstElementChild.src
      ).returnObject()
    ];
  });
};
...
saveFootMark.addEventListener("click", () => {
  questionCreator();
  localForage
    .setItem("questions", { qeustion: questionRack, data: plain.innerText })
    .then(i => {
      console.log("done");
    });
  console.log(questionRack);
});
saverBtn.addEventListener("click", () => {
  questionCreator();
  localForage
    .setItem("questions", { question: questionRack, data: plain.innerText })
    .then(i => {
      console.log("done");
    });
});
